...
static inline void model_tick(struct Model* model) {
    switch(model->phase) {
    case RedToGreenUp:
...
static inline void tick(struct AppView* view, struct Model* model) {
    model_tick(model);
...
static void handle_timer(void* ctx) {
    ...
    with_view_model(
        view->view, struct Model * model, { tick(view, model); }, true);
}

static inline void model_init(struct Model* model) {
    model->phase = RedToGreenUp;
    model->led_red = 255;
    model->led_green = 0;
    model->led_blue = 0;
...
static void handle_alloc(void* ctx) {
    furi_assert(ctx);
    struct AppView* view = ctx;
    view->localctx = malloc(sizeof(struct LocalCtx));
    struct LocalCtx* localctx = view->localctx;
    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(struct Model));
    with_view_model(
        view->view, struct Model * model, { model_init(model); }, true);
...
static void handle_enter(void* ctx) {
    ...
    struct LocalCtx* localctx = (struct LocalCtx*)view->localctx;
...
struct ViewConfig view_led_rainbow_config = {
    .id = ViewLEDRainbow,
